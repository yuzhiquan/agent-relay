...
坦诚地说：它是一个**协调层（coordination layer）**，而不是一个 agent。真正的推理发生在
它调用的各个 CLI 内部，agent-relay 只是把它们串接起来。
...
- **交接是基于文本的。** 各步骤通过输出文件和 `VERDICT:` 标记来协调，简单但脆弱——
  ...
  参见 [`PLAN.md`](PLAN.md)。
- **单一厂商的工具正在迎头赶上。** Claude Code 的 subagents、先规划再确认的模式、
  以及 hooks，已经能原生覆盖「在单个工具内」的场景。真正持久的差异化优势是
  **跨厂商（cross-vendor）**编排——例如*用 Claude 规划、用 Codex 实现*——
  这是任何单一厂商的 subagent 体系都做不到的。
